	...
	if (m_cfg->isGeoLoaded())
	{
		if (!cross_data.empty())
			for (int i = 0; i < cross_data.size(); i++)
			{
				...
				switch (m_selected_plane) {
				case PlaneID::OYZ:
					cross_points.push_back(wxPoint(cross_point.GetY() * m_cfg->getSizeX() / 2 , cross_point.GetZ() * m_cfg->getSizeY() / 2));
					break;
				case PlaneID::OXZ:
					cross_points.push_back(wxPoint(cross_point.GetX() * m_cfg->getSizeX() / 2 , cross_point.GetZ() * m_cfg->getSizeY() /2 ));
					break;
				case PlaneID::OXY:
					cross_points.push_back(wxPoint(cross_point.GetX() * m_cfg->getSizeX() / 2, cross_point.GetY() * m_cfg->getSizeY() / 2));
					break;
				}
			}
